...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/31 03:57:21 by oorlov            #+#    #+#             */
/*   Updated: 2019/07/31 03:57:23 by oorlov           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		init_buffer(t_buff **buffer, t_arg *arg)
{
	if (flag(arg, '0')
		...
		(*buffer)->buff = ft_set_sym('0', (*buffer)->buff_size);
	else
		(*buffer)->buff = ft_set_sym(' ', (*buffer)->buff_size);
}
...
t_buff			*percent_buffer(char *format, va_list params)
{
	...
	buff_value(&buffer, arg);
	set_buff_sup(&buffer, arg);
	buff_size(&buffer, arg);
